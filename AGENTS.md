...
# 用 pyproject.toml 中的版本发布文档并让 latest 指向它（内部执行 mike deploy --update-aliases --push）
uv run docs deploy
```
...
- `docs.yml` 在 `main` 上文档、素材或版本变化时直接调用 `mike deploy --update-aliases --push <版本号> latest`，`latest` 指向当前版本；mike 默认只提交不推送，必须带 `--push`
